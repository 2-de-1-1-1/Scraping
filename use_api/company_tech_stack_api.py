...
class JobApiFetcher:
    def __init__(self, start_page, end_page, data_folder='api_data', output_file='technical_data.json'):
    ...
    def fetch_company_details(self, company_id):
        ...
        if response.status_code == 200:
            ...
            print(f"Failed to fetch company technical tags for company ID {company_id}. Status code: {response.status_code}")
            return []
            ...
            if job_data:
                for job in job_data['jobPositions']:
                    ...
                    if technical_tags is not None:
                        all_technical_data.append({
                            'company_id': company_id,
                            'tech_stack': technical_tags
                        })

        with open(self.output_file, 'w', encoding='utf-8') as file:
            json.dump(all_technical_data, file, ensure_ascii=False, indent=4)
...
if __name__ == "__main__":
    ...
    output_file = 'company_tech_stack_api.json'
